...
// --- Recorder: record -> review -> save -> run ---

const LANE_BASE = "/api/lanes/scb-business-anywhere-1";
let activeRecordingRunId = null;
...
function setRecordingUi(recording) {
  ...
  document.getElementById("recording-start-btn").disabled = recording;
  document.getElementById("recording-stop-btn").disabled = !recording;
}
...
function showRecordingResult(result) {
  ...
  const listEl = document.getElementById("recording-steps-list");
  listEl.textContent = result.steps.map((s, i) => `${i + 1}. ${describeCompiledStep(s)}`).join("\n") || "(no actions captured)";
  const hintEl = document.getElementById("recording-redacted-hint");
  hintEl.textContent =
  ...
  document.getElementById("recording-review").style.display = "block";
}
...
async function pollRecordingJob() {
  ...
  try {
    ...
    const data = await res.json();
    const job = (data.jobs || []).find((j) => j.id === activeRecordingJobId);
    if (!job) return;
...
document.getElementById("recording-start-btn").addEventListener("click", async () => {
  showError(null);
  document.getElementById("recording-review").style.display = "none";
  try {
    ...
    activeRecordingRunId = data.runId;
    activeRecordingJobId = data.jobId;
    setRecordingUi(true);
    recordingPollTimer = setInterval(pollRecordingJob, 3000);
  } catch (err) {
    showError(`Request failed: ${err}`);
  }
});
...
document.getElementById("recording-stop-btn").addEventListener("click", async () => {
  if (!activeRecordingRunId) return;
  try {
    await fetch(`${LANE_BASE}/recordings/stop`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ runId: activeRecordingRunId }),
    });
    document.getElementById("recording-stop-btn").disabled = true;
    document.getElementById("recording-status-label").textContent = "stopping…";
  } catch (err) {
    showError(`Request failed: ${err}`);
  }
});

document.getElementById("recording-save-btn").addEventListener("click", async () => {
  showError(null);
  const name = document.getElementById("recording-save-name").value.trim();
  ...
  try {
    ...
    document.getElementById("recording-review").style.display = "none";
    document.getElementById("recording-save-name").value = "";
...
async function fetchReplayState() {
  try {
    ...
    const data = await res.json();
    const banner = document.getElementById("pending-confirmation-banner");
