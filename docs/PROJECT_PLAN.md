...
| MinIO/S3 scope | Screenshots only, uploaded to a single `weboperator-artifacts` bucket under `screenshots/<filename>` | Matches the explicit instruction to start from screenshot artifacts. Session-file archival (`data/sessions/*`) and wiring MinIO into the Control Panel UI/API (still reads local files only) are separate, natural follow-ups, not bundled into this change. |
...
- [x] ดาวน์โหลดและจัดเก็บข้อมูล (MinIO/S3) — partial: `services/worker/src/artifacts.ts` (`uploadArtifact`) mirrors every screenshot (fixed scripts + generic `screenshot` workflow action) to a new `minio` compose service (`weboperator-artifacts` bucket, `screenshots/*`), added alongside the existing local files, not replacing them. Best-effort via new `stepBestEffort()` — an upload failure shows as an `archive-screenshot` step error but never fails the job. Verified: worker→MinIO connectivity through Docker service DNS, a genuine upload+fetch-back round trip with matching SHA-256 hashes, non-fatal behavior with MinIO stopped, and `archive-screenshot` showing up correctly through the real Control Panel queue (`the-internet-login` workflow) with the existing `/screenshots/*` route unaffected. Downloads/session-file archival/Control-Panel-side retrieval from MinIO are not done yet.
...
retry, a generic multi-action workflow engine proven against one real
example workflow, and screenshot artifacts now mirrored to MinIO/S3
best-effort. Still open: migrating the 4 fixed actions onto the workflow
engine (optional consolidation, not required), extending MinIO archival to
downloads/session files, wiring the Control Panel to read artifacts back
from MinIO instead of only local disk, and — if it ever becomes worth the
